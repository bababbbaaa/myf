...
var obj = $json;
    ...
    $('.tps1b1').on('click', function() {
        var infoTitle = $('.tps1b1').attr('data-title');
        $('.tps1').fadeOut(300, function() {
            $('.popup__title__section').val('Технология: ' + infoTitle);
            $('.tps2').fadeIn(300);
        });
    });
...
<div class="technoback1">
  <div class="technopop tps1">
    ...
    <div class="pow">
      <button type="button" data-title="" class="orangeLinkBtn tps1b1">Получить</button>
  ...
  <div class="technopop tps2 dnone">
    ...
    <div class="giveYsl">
      ...
      <input class="popup__title__section" type="hidden" name="section" value="">
...
<section class="CatSec1">
  <div class="container">
    <div class="technologies">
      <a id="catalogtechnolog"></a>
      <h1 class="CatH1">Технологии FEMIDA.FORCE</h1>
      <p class="FSec8P1">К любому пакету франшизы вы можете приобрести дополнительные услуги</p>
      <div class="FSHelps">
        <?= Html::beginForm('technologies', 'get', ['class' => 'filt__form']) ?>
        ...
        <div class="Franchaizes">
          <?php if (!empty($franchaiz)) : ?>
            <?php foreach ($franchaiz as $k => $item) : ?>
              <div data-click="<?= $item['id'] ?>" class="FSHelp HovPopup <?php if ($k == 0 || $k % 4 == 0) : ?>FSHelp_color<?php elseif ($k % 4 == 1) : ?>FSHelp_color1 <?php elseif ($k % 4 == 2) : ?>FSHelp_color2 <?php elseif ($k % 4 == 3) : ?>FSHelp_color3<?php endif; ?>">
                <img src="<?= Url::to(['/img/settings1.svg']) ?>" alt="icon settings">
                <h5><?= $item['name'] ?></h5>
                <p><?= $item['preview'] ?></p>
              </div>
            <?php endforeach; ?>
