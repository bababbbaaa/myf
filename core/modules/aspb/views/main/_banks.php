    ...
    <?php if (!empty($popupdata->banks) && $popupdata->banks !== '[]'): ?>
        <?php foreach (json_decode($popupdata->banks, 1) as $k => $v): ?>
            ...
            <div class="banks-popup-row-item">
                <button data-id="<?= $popupdata->id ?>" data-bank="<?= $k ?>" class="banks-popup-row-item--btn banks--remove">
...
<div class="banks--set-card-inputs">
    <input id="bank_count" style="width: 48%;" class="input-case input-t"
           type="number" placeholder="Счёт" name="bank_count">
    <input id="" style="width: 48%;" class="input-case input-t"
           type="number" placeholder="БИК" name="bank_bic">
    <input id="" style="max-width: 100%;" class="input-case input-t" type="text"
           placeholder="Банк" name="bank_name">
</div>

<button data-id="<?= $popupdata->id ?>" class="btn--orange banks--set-add">Добавить</button>