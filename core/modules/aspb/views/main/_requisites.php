    ...
    <?php if (!empty($popupdata->requisites) && $popupdata->requisites !== '[]'): ?>
        <?php foreach (json_decode($popupdata->requisites, 1) as $k => $v): ?>
            ...
            <div class="banks-popup-row-item">
                <button data-id="<?= $popupdata->id ?>" data-bank="<?= $k ?>" class="banks-popup-row-item--btn requisites--remove">
...
<div class="banks--set-card-inputs">
    <input style="width: 48%;" class="input-case input-t"
           type="number" placeholder="Счёт" name="requisites_count">
    <input style="width: 48%;" class="input-case input-t"
           type="number" placeholder="БИК" name="requisites_bic">
    <input style="max-width: 100%;" class="input-case input-t" type="text"
           placeholder="Банк" name="requisites_name">
    <input style="max-width: 100%;" class="input-case input-t" type="text"
           placeholder="ФИО" name="requisites_fio"">
</div>

<button data-id="<?= $popupdata->id ?>" class="btn--orange requisites--set-add">Добавить</button>