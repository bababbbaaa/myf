...
function parseAdvs() {
    ...
    html += '</div>';
    advBlock.html(html);
    $('#leadtemplates-advantages').val(JSON.stringify(advantages));
}
parseAdvs();
...
<div class="lead-types-form">
    ...
    <div style="display: none">
        <?= $form->field($model, 'category_link')->textInput(['maxlength' => true, 'class' => 'changeCatNameThis', 'value' => !empty($model->category_link) ? $model->category_link : 'dolgi']) ?>
    </div>

    <?= $form->field($model, 'price')->input('number', ['placeholder' => '500']) ?>
    ...
    echo $form->field($model, 'image')->widget(InputFile::className(), [
        ...
        'options'       => ['class' => 'form-control', 'placeholder' => 'Иконка шаблона'],
    ...
    <div class="align-to-area " data-type="chardjbek" style="margin-bottom: 20px; display: <?= !empty($model->category_link) && $model->category_link === 'chardjbek' ? 'block' : 'none' ?>">
        ...
        <p>
            ...
            <select class="form-control chosen-select" name="LeadTemplates[geo]" id="">
    ...
    <div class="rbac-info rbac-info-leads" style="max-width: unset">
        <p><code>Название</code> - то название, которое будет видно пользователям при выборе шаблона на лендинге или в ЛК</p>
        <p><code>Категория</code> - категория поставляемых лидов</p>
        <p><code>Цена</code> - стандартная цена за полученный лид, которая будет списана с баланса клиента</p>
        <p><code>Описание</code> - любое текстовое описание шаблона заказа</p>
        ...
        <p><code>Особенности</code> - короткие текстовые тезисы, для описания поставляемого формата лидов. Данные параметры ни на что не влияют и нужны только для отображения на лендинге</p>
        <p><code>Активность шаблон</code> - указать, является ли данный шаблон публичным для ЛК и лендинга</p>
