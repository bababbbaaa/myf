...
$('.use-action-popup').on('click', function(e) {
    $.ajax({
        data: {data: ids, type: type, typeLead: $('.typeLead').val(), sourceLead: $('.sourceLead').val(), typeBackdoor: 'deal'},
        dataType: 'JSON',
        type: "POST",
...
<hr>
<div>
    <h1>Сделки с бекдора</h1>
    <div>
        <h3>Фильтры</h3>
        <?= \yii\helpers\Html::beginForm('backdoor-deals', 'GET', ['name' => 'form__filter']); ?>
    ...
    <div style="margin-top: 30px">
        <?php if(!empty($models)): ?>
            ...
            <table class="table table-bordered table-responsive">
                ...
                <tr class="<?= in_array($item['source'], $rArr) ? 'active-order-backdoor' : '' ?>">
                    ...
                    <td><?= !empty($item['summ']) ? $item['summ'] : '-' ?></td>
        ...
        <?php else: ?>
        <p style="color: #9e9e9e">Сделки с бекдора не найдены</p>
        <?php endif; ?>
    </div>
</div>
