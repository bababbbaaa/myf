...
$js = <<< JS
    ...
    $('.MyOrders_filter-reset').on('click', function() {
        location.href = '/lead-force/client/myorders#'+ hash;
    });
        ...
        <section class="OrderCardS OrderPage OrderPage-2">
            ...
            <div class="MyOrders_filter">
                ...
                <select class="MyOrders_filter-select" name="moderationFilter[sphere]" id="">
                    <option selected disabled>Сфера</option>
                    <?php foreach ($sphere as $k => $v): ?>
                        <option class="sendModeration" value="<?= $v['name'] ?>"><?= $v['name'] ?></option>
                    <?php endforeach; ?>
                </select>
                <input class="MyOrders_filter-check" type="checkbox" name="moderationFilter[new]"
                       id="MyOrders_filter-check1">
                <label class="MyOrders_filter-check-l sendModeration <?= !empty($_GET['moderationFilter']['new']) ? 'activeCheck' : '' ?>"
                       for="MyOrders_filter-check1">Новые</label>
            </div>
            <?= Html::endForm(); ?>
...
<!--                        </div>-->
            ...
            <?php else: ?>
                <section class="rightInfo_no-orders">
                    <div class="title_row">
                        <p class="Bal-ttl title-main">Заказы на модерации</p>
                    </div>
                    <section class="rightInfo_no-orders_info">
                        <img class="rightInfo_no-orders_info-back"
                             src="<?= Url::to(['/img/rightInfo_no-orders-back.svg']) ?>" alt="иконка">
            ...
            <div class="OrderCard StatisticsCard">
                ...
                <div class="OrderPage_StatisticsFilter_top">
                    <label class="OrderPage_StatisticsFilter_top-label">
                        <input checked class="OrderPage_StatisticsFilter_top-input generalStatistickReload" type="radio" name="type"
                    ...
                    <div class="OrderPage_StatisticsFilter_bottom">
                        ...
                        <select class="OrderPage_StatisticsFilter-Select" name="orderId" id="orderId">
                            <?php foreach ($countOrdersId as $k => $v): ?>
                                <option class="sendIdOrder" <?= $_GET['orderId'] == $v['id'] ? 'selected' : '' ?> value="<?= $v['id'] ?>"><?= !empty($v['order_name']) ? $v['order_name'] : "Заказ № {$v['id']} {$v['category_text']}" ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                <?php endif; ?>
...
JS;
        ...
        <section class="OrderCardS OrderPage OrderPage-4">
