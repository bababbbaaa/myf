...
//    }
    ...
    public function actionIndex()
    {
        ...
        $bonuses = UsersBonuses::find()->where(['user_id' => $user->getId()])->asArray()->one();
        /* Статистика */
//        $budget = BudgetLog::find()->where(['user_id' => $user->getId()])->andWhere('date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY)')->asArray()->all();
        ...
        if (!empty($client)) {
            ...
            if (!empty($_GET['moderationFilter'])) {
                ...
                if (!empty($moderationGet['sphere'])) {
                    $moderationFilter[] = ['=', 'category_text', $moderationGet['sphere']];
                }
                # новые заказы
            ...
            if (!empty($client)) {
                ...
                if (!empty($order)) {
                    if ($order->status == Orders::STATUS_PROCESSING || $order->status == Orders::STATUS_PAUSE) {
                        ...
                        if ($waste > $leadWaste) {
                            if ($why === LeadsSentReport::REASON_WRONG_REGION || $why === LeadsSentReport::REASON_WRONG_PHONE || $why === LeadsSentReport::REASON_NOT_LEAD || $why === LeadsSentReport::REASON_DUPLICATE || mb_strlen($why) > 4) {
                                $leads = LeadsSentReport::find()
                                    ->where(['in', 'lead_id', $id])
                                    ->andWhere(['client_id' => $client['id'], 'order_id' => $order->id])
                                    ->andWhere(['status' => Leads::STATUS_SENT])
                                ...
                                if (!empty($leads)) {
                                    foreach ($leads as $item) {
                                        ...
                                        return $functions->wasteLead($item->id);
                                    }
                                    $rsp = ['status' => 'success'];
                                } else $rsp = ['status' => 'error', 'message' => 'Лиды не найдены'];
                            } else $rsp = ['status' => 'error', 'message' => 'Указана не верная причина отбраковки'];
    ...
    public function actionUsermanualauction()
    {
        return $this->render('usermanualauction');
    }

    public function actionUsermanualbalance()
    {
        return $this->render('usermanualbalance');
    }
    ...
    public function actionUsermanualmyorders()
    {
        return $this->render('usermanualmyorders');
    }
    ...
    {
        $user = Yii::$app->getUser()->getId();
        $userInfo = User::find()->where(['id' => $user])->asArray()->one();
        if (!empty($user)) {
            $bonuses = UsersBonuses::findOne(['user_id' => $user]);
            if (!empty($bonuses)) {
                $info = $bonuses->checkInfo();
            } else {
                $info = ['info' => 'Закажите 50 лидов и получите кэшбек 3% на все последующие заказы!'];
            }
        } else $info = ['Пользователь не найден'];
    ...
    public function actionTestButch()
    {
//        $user_id = Yii::$app->getUser()->getId();
//        Yii::$app->response->format = Response::FORMAT_JSON;
//        $post = [
...
//            'halt' => 0,
//            'cmd'  => [
//                'deal' => "crm.deal.list?filter[UF_CRM_1594804059623]={$user_id}&filter[CATEGORY_ID]=104",
//                'item' => 'crm.item.update?entityTypeId=172&id=50&' . http_build_query([
//                        'fields' => [
//                            'TITLE'                 => "ТЕстирую",
//                            'parentId2'             => '$result[deal][0][ID]',
//                            'UFCRM1650861498940'  => 250,
//                            'UF_CRM_1650861513854'  => 'rostov',
//                            'UF_CRM_1650861526188'  => 'bla-bla',
//                            'UF_CRM_1612930698601'  => 308,
//                            'ufCrm1612930706779'  => 25,
//                            'ASSIGNED_BY_ID'        => 398,
//                            'STAGE_ID'              => 'DT172_8:PREPARATION'
//                        ]
//                    ]),
//            ],
//        ];
//        $bitrix = new Bitrix();
//        return $respone = $bitrix->set__method('batch')->set__params($batch)->process();
    }
}
