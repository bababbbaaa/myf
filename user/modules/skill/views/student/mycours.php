            ...
            <div class="mycours_top">
                <div class="mycours_top-left">
                    ...
                    <?php if ($course->price == 0) : ?>
                        <p class="mycours_top-left-freecourse">Бесплатный курс</p>
                    <?php endif; ?>
                </div>
                                    ...
                                    <?php if (!empty($courseArr)) : ?>
                                        <?php foreach ($courseArr as $key => $val) : ?>
                                            <?php foreach ($val as $ke => $item) : ?>
                                                <?php if (array_key_exists('main_text', $item)) : ?>
                                                    <?php if ($v['id'] == $item['block_id']) : ?>
                                                        <div class="mycours-list-item_info-item">
                                                            <a class="courses__links" href="<?= Url::to(['lesson', 'id' => $item['id']]) ?>"></a>
                                                            <div style="background-image: url(<?= Url::to(['/img/skillclient/ppt.png']) ?>)" class="mycours-list-item_info-item-bacground-lesson">
                                                                <?= $item['sort_order'] ?>
                                                                <p>Теория</p>
                                                            </div>
                                                            <h4 class="mycours-list-item_info-item-name"><?= $k + 1 ?>
                                                                .<?= $item['sort_order'] ?> <?= $item['name'] ?></h4>
                                                        </div>
                                                    <?php endif; ?>
                                                <?php elseif (array_key_exists('max_tries', $item)) : ?>
                                                    <?php if ($v['id'] == $item['block_id']) : ?>
                                                        <div class="mycours-list-item_info-item">
                                                            ...
                                                            <a class="courses__links" href="<?= Url::to(['test', 'id' => $item['id']]) ?>"></a>
                                                            <div style="background-image: url(<?= Url::to(['/img/skillclient/full-selection.png']) ?>)" class="mycours-list-item_info-item-bacground-test">
                                                                <?= $item['sort_order'] ?>
                                                                <p>Тест</p>
                                                            </div>
