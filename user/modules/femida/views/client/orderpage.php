...
$js = <<< JS
    ...
    $('.Popap-order-BTN').on('click', function () {
        var why = $('.leads__Waste__why option:selected').val(),
            id = $('.wasteLead').attr('data-id'),
...
<section class="rightInfo">
  ...
  <nav class="BredCramsonBalance df aic">
    <a href="<?= Url::to(['promotion']) ?>" class="BRChome uscp MText">Мои заказы</a>
    <img src="<?= Url::to(['/img/ArrRightBRC.png']) ?>" alt="Стрелка справа">
    <a href="<?= Url::to(['promotion']) ?>" class="BRChome uscp MText">Активные заказы</a>
    <img src="<?= Url::to(['/img/ArrRightBRC.png']) ?>" alt="Стрелка справа">
  ...
  <?php if ($order['price'] > $user['budget']) : ?>
    <section class="order-page_alert-zero-balance">
      ...
      <div class="order-page_alert-zero-balance_info">
        ...
        <p class="order-page_alert-zero-balance_info-text">Ваш заказ <?= $order['order_name'] ?> начнет исполняться после того, как вы
          пополните баланс</p>
  ...
  <section class="order-page_info-cards">
        ...
        <div class="order-page_info-card-1_top">
          ...
          <div class="order-page_info-card-1_actions">
            <?php if ($order['status'] == 'исполняется') : ?>
              <div class="dropdown">
                <a href="#" data-toggle="dropdown" class="dropdown-toggle">Действия <b class="caret"></b></a>
                <?php
    ...
    <div class="order-page_info-cards_right">
      ...
      <div class="order-page_info-cards_right-week">
        ...
        <?php foreach ($days as $k => $v) : ?>
          ...
          <?php if (!empty($params['days_of_week_leadgain'])) : ?>
            ...
            <?php endif; ?>
          <?php endif; ?>
          <div data-day="<?= $k + 1 ?>" class="order-page_info-cards_right-day <?= $class ?>">
            <?= $v ?>
          </div>
        <?php endforeach; ?>
      </div>
      ...
      <div class="order-page_info-cards_right_time">
        ...
        <input type="text" name="time_start" value="<?= $params['start_time_leadgain'] ?>" class="timepick start__time">
        <p class="order-page_info-cards_right_time-t2"></p>
        <p class="order-page_info-cards_right_time-t1">до</p>
        <input type="text" name="time_end" value="<?= $params['end_time_leadgain'] ?>" class="timepick end__time">
      </div>
      ...
      <div class="order-page_info-cards_right_process">
        <div class="order-page_info-cards_right_process_top">
          <div class="order-page_info-cards_right_process">
            <p>Отгружено лидов:<span class="order-page_info-cards_right_process-percentage"><?= round(($order['leads_get'] * 100) / $order['leads_count'], 2) ?>%</span>
            </p>
          </div>
          <div class="order-page_info-cards_right_process-bott">
            <div style="width: <?//= round(($order['leads_get'] * 100) / $order['leads_count'], 2) ?>px" class="order-page_info-cards_right_process-bottfill"></div>
          </div>
        </div>
        <div class="order-page_info-cards_right_process_top">
          <div class="order-page_info-cards_right_process">
            <p>Отбраковано лидов:<span class="order-page_info-cards_right_process-percentage"><?= round(($order['leads_waste'] * 100) / ($order['leads_count'] - $order['leads_waste']), 2) ?>%</span>
            </p>
          </div>
          <div class="order-page_info-cards_right_process-bott">
            <div style="width: <?= $order['waste'] + $bonus_waste > 0 ? round(100 / ($order['waste'] + $bonus_waste), 0) : 2 ?>%;" class="order-page_info-cards_right_process-bottfill order-page_info-cards_right_process-bottfill2"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <?php else : ?>
    ...
    <section class="order-page_leads_wrapp">
      <div class="order-page_leads-tabs">
        <div data-tab="all" class="order-page_leads-tab <?= empty($_GET['tab']) ? 'active' : '' ?> <?= $_GET['tab'] == 'all' ? 'active' : '' ?>" id="order-page_leads-tab1">
          <p class="order-page_leads-tab-t1">Все лиды</p>
          <p class="order-page_leads-tab-t2"><?= $allLead ?></p>
        </div>
        <div data-tab="send" class="order-page_leads-tab <?= $_GET['tab'] == 'send' ? 'active' : '' ?>" id="order-page_leads-tab2">
          <p class="order-page_leads-tab-t1">Новые лиды</p>
          <p class="order-page_leads-tab-t2">+<?= $newLead ?></p>
        </div>
        <div data-tab="moderation" class="order-page_leads-tab <?= $_GET['tab'] == 'moderation' ? 'active' : '' ?>" id="order-page_leads-tab3">
          <p class="order-page_leads-tab-t1">Отправлено в брак</p>
        </div>
        <div data-tab="waste" class="order-page_leads-tab <?= $_GET['tab'] == 'waste' ? 'active' : '' ?>" id="order-page_leads-tab4">
      ...
      <div class="order-page_leads">
        <div class="order-page_leads_tab order-page_leads_tab1">
          ...
          <div class="order-page_leads_filter adaptiveFilter_lead">
            <input placeholder="Ключевое слово" autocomplete="off" class="InputSearch adInpSearch" type="text" name="filters[word]">
            ...
            <button type="submit" class="uscp Page2-Balance_chooseP-BTN df aic jcc order-page_leads_filterBTN adFiltersBtn">Показать
            </button>
          </div>
          ...
          <?php foreach ($leads as $k => $v) : ?>
              ...
              <?php if (!empty($v->filteredLeads)) : ?>
                <div class="order-page_leads_tab_row">
                  <?php if ($v->status == \common\models\Leads::STATUS_SENT) : ?>
                    <input class="leads_filter_checkbox select__lead" type="checkbox" name="id[]" value="<?= $v->filteredLeads->id ?>">
                  <?php else : ?>
                  ...
                  <div data-id="<?= $v->filteredLeads->id ?>" data-pjax="0" class="order-page_leads_link">
            ...
            <?php else : ?>
              <div class="order-page_leads_tab_row">
                <?php if ($v->status == \common\models\Leads::STATUS_SENT) : ?>
                  <input class="leads_filter_checkbox select__lead" type="checkbox" name="id[]" value="<?= $v->lead->id ?>">
                <?php else : ?>
                  <div style="width: 20px; padding: 9px;"></div>
                <?php endif; ?>
